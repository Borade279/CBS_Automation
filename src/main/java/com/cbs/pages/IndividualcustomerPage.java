...
public class IndividualcustomerPage extends TestBase 
{
	...
	String location = "C:\\Master_4.0\\Master_4.0_Automation.xlsx";
	TestUtil util;
	...
	@FindBy(xpath= "//button[@onclick='update(0)']")
	//@FindBy(xpath="//table[@class='table table-striped table-bordered']/tbody/tr[1]/td[23]/button[contains(text(),'View')]")
	WebElement View;
	
	@FindBy(xpath ="//button[@id='btnVerify_0']")
	//@FindBy(xpath="//table[@class='table table-striped table-bordered']/tbody/tr[1]/td[24]/button[contains(text(),'Verify')]")
	WebElement Verify;
	...
	public void createIndividualCustomer() throws Exception
	{
		...
		for (int i=1;i<=rowcount;i++)
		{
			...
			{
				int PAN = (int)(Math.random()*(max-min+1)+min);
				Pan_No.sendKeys("AADPB"+PAN+"Z");
				int Adhar = (int)(Math.random()*(max-min+1)+min);
				Aadhar_No.sendKeys(Adhar+"12345688");
				Thread.sleep(2000);
				...
				//int aa = (int)(Math.random()*(maxName-minName+1)+minName);				
				First_name.sendKeys(row.getCell(2).getStringCellValue()); //int aa taken only to take new name from excel file, if any difficulty then use index from excell sheet //2
				Middle_name.sendKeys(row.getCell(3).getStringCellValue()); // use this index ---> 3
				Last_name.sendKeys(row.getCell(4).getStringCellValue());  // use this index ---> 4
				...
				Father_First_name.sendKeys(row.getCell(5).getStringCellValue());//use this index --->5
				Father_Middle_name.sendKeys(row.getCell(6).getStringCellValue());//use this index --->6
				Father_Last_name.sendKeys(row.getCell(7).getStringCellValue());//use this index --->7
				...
				Mother_First_name.sendKeys(row.getCell(8).getStringCellValue());//use this index --->8
				Mother_Middle_name.sendKeys(row.getCell(9).getStringCellValue());//use this index --->9
				Mother_Last_name.sendKeys(row.getCell(10).getStringCellValue());//use this index --->10
				//int dob = (int) row.getCell(11).getNumericCellValue();
				//DOB.sendKeys(String.valueOf(dob));
				Thread.sleep(2000);
				DOB.sendKeys("01/1992/01");
				//CIBIL_Score
				...
				driver.findElement(By.id("id_txtEmployeeId")).click();
				Thread.sleep(3000);
				...
				driver.findElement(By.id("id_txtDefaultRiskChangeDate")).click();
				Thread.sleep(1000);
				...
				driver.findElement(By.xpath("//input[@id='id_threshold']")).sendKeys("9999999");
				//Minority
				//Weaker
				//Place of Birth
